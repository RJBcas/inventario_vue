                       ...
                       <table id="example" class="table table-striped table-bordered">
							...
							<nav aria-label="Page navigation">
								<ul class="pagination">
									...
									<li v-for="pageNumber in totalPage" v-if="Math.abs(pageNumber - currentPage) < 3 || pageNumber == totalPage - 1 || pageNumber == 0" :class="{ 'active' : currentPage === pageNumber }">
										<a @click="setPage(pageNumber)" :class="{ 'active' : currentPage === pageNumber, last: (pageNumber == totalPage - 1 && Math.abs(pageNumber - currentPage) > 3), first:(pageNumber == 0 && Math.abs(pageNumber - currentPage) > 3)}"> <span>{{ pageNumber+1 }}<span v-if="currentPage === pageNumber" class="se-only">(current)</span></span></a>
									</li>
									<li>
